class Solution(object):
    def smallestNumber(self, num, t):
        ...
        n, has_zero, first_zero = len(num), False, len(num)
        for idx, char in enumerate(num):
            if char == '0':
                has_zero, first_zero = True, idx
                break
        if not has_zero:
            r2, r3, r5, r7 = req2, req3, req5, req7
            for char in num:
                d = int(char)
                r2, r3, r5, r7 = max(0, r2 - F2[d]), max(0, r3 - F3[d]), max(0, r5 - F5[d]), max(0, r7 - F7[d])
            if r2 == 0 and r3 == 0 and r5 == 0 and r7 == 0: return num
        limit = min(n - 1, first_zero)
        p2 = p3 = p5 = p7 = 0
        for i in range(limit):
            d = int(num[i])
            p2, p3, p5, p7 = p2 + F2[d], p3 + F3[d], p5 + F5[d], p7 + F7[d]
        for i in range(limit, -1, -1):
            start_d = int(num[i]) + 1
            for d in range(start_d, 10):
                n2, n3, n5, n7 = max(0, req2 - p2 - F2[d]), max(0, req3 - p3 - F3[d]), max(0, req5 - p5 - F5[d]), max(0, req7 - p7 - F7[d])
                L = n - 1 - i
                if n7 + n5 + dp[n2][n3] <= L:
                    ...
                    for pos in range(L):
                        for x in range(1, 10):
                            nn2, nn3, nn5, nn7 = max(0, rem2 - F2[x]), max(0, rem3 - F3[x]), max(0, rem5 - F5[x]), max(0, rem7 - F7[x])
        ...
        ans_list, rem2, rem3, rem5, rem7 = [], req2, req3, req5, req7
        for pos in range(M):
            for x in range(1, 10):
                nn2, nn3, nn5, nn7 = max(0, rem2 - F2[x]), max(0, rem3 - F3[x]), max(0, rem5 - F5[x]), max(0, rem7 - F7[x])
