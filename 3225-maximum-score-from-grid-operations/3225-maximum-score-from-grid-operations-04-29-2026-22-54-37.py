class Solution:
    def maximumScore(self, grid: List[List[int]]) -> int:
        n = len(grid[0])
        if n == 1: return 0
        dp = [[[0] * (n + 1) for i in range(n + 1)] for j in range(n)]
        prev_max = [[0] * (n + 1) for i in range(n + 1)]
        prev_suffix_max = [[0] * (n + 1) for i in range(n + 1)]
        col_sum = [[0] * (n + 1) for i in range(n)]
        for c in range(n):
            ...
            for curr_h in range(n + 1):
                for prev_h in range(n + 1):
                    if curr_h <= prev_h:
                        ...
                        dp[i][curr_h][prev_h] = max(dp[i][curr_h][prev_h],prev_suffix_max[prev_h][0] + extra_score,)
                    else:
                        extra_score = (col_sum[i - 1][curr_h] - col_sum[i - 1][prev_h])
                        dp[i][curr_h][prev_h] = max(dp[i][curr_h][prev_h],prev_suffix_max[prev_h][curr_h],prev_max[prev_h][curr_h] + extra_score,)
            for curr_h in range(n + 1):
